...
async function createUsers(req, res, next) {
    const { name, lastName, rol, document, birthday, nationality, country, province, cp, location, address } = req.body;
    try {
        if (rol === "Alumno" || rol === "Piloto" || rol === "Instructor" || rol === "InstructorAdmin" || rol === "Admin") {
            try {
                const users = await Users.create({
                    name, lastName, rol, document, birthday, nationality, country, province, cp, location, address
                });
